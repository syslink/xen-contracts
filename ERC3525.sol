...
contract ERC3525 is IERC3525, IERC3525Metadata, ERC721Enumerable {
    ...
    function transferFrom( uint256 fromTokenId_, address to_, uint256 value_) public payable virtual override returns (uint256) {
        ...
        _slotTokens[_slots[fromTokenId_]].add(newTokenId);
    ...
    function _afterValueTransfer( address from_, address to_, uint256 fromTokenId_, uint256 toTokenId_, uint256 slot_, uint256 value_) internal virtual {
        if (from_ == address(0) && fromTokenId_ == 0 && !_slotTokens[slot_].contains(toTokenId_)) {
            _slotTokens[slot_].add(toTokenId_);
        } else if (to_ == address(0) && toTokenId_ == 0 && _slotTokens[slot_].contains(toTokenId_)) {
            _slotTokens[slot_].remove(fromTokenId_);
        }
    }
    ...
    function tokenInSlotByIndex(uint256 slot_, uint256 index_) public view returns (uint256) {
        return _slotTokens[slot_].at(index_);
    }

    function allTokensInSlot(uint256 slot_) public view returns (uint256[] memory) {
        return _slotTokens[slot_].values();
    }
}
